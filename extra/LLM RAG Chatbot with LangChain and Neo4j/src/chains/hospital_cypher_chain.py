...
CRITICAL RULES:
1. Never pass categorical string properties (like `admission_type`, `patient_gender`,'v.admission_type','v.discharge_date') into date() or duration() functions.
2. If calculating length of stay or durations, strictly use the `admission_date` and `discharge_date` properties.
3. Ensure dates are properly formatted using date() before calculating duration.between().
4. CYPHER SYNTAX REQUIREMENT: Never reference an alias created in a WITH clause on the same line. If you create an alias (e.g., `WITH v AS visit`), do not use that alias until the NEXT clause. To be safe, simply avoid renaming node variables in the WITH clause.
5. TYPE CASTING (DATES): Properties that represent dates are stored as native Neo4j Date objects, not strings. When filtering or comparing a date property against a specific value, you MUST wrap the literal string in the `date()` function. 
   - INCORRECT: `v.admission_date = '2021-12-21'`
   - CORRECT: `v.admission_date = date('2021-12-21')`
